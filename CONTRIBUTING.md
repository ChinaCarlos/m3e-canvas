...
- Run `pnpm typecheck`, `pnpm test` and `pnpm build`; CI runs the same three on every PR.
...
- PR の前に `pnpm typecheck`、`pnpm test`、`pnpm build` を通してください。CI でも同じものが走ります。
...
- 提交 PR 前请运行 `pnpm typecheck`、`pnpm test` 和 `pnpm build`，CI 会执行同样的检查。
- 贡献的代码以 MIT 许可证发布。
...
- PR 전에 `pnpm typecheck`, `pnpm test`, `pnpm build`를 통과시켜 주세요. CI에서도 같은 검사가 실행됩니다.
